...
it("lokal preflight bevarer dual-read, revocation og kopiparitet uden deploy", () => {
  ...
  // serverejede serviceområder tilføjer seks læseregler, men ingen direkte
  // klientskrivning. Fælles ressourcekategorier og hardware-registre
  // tilføjer fire tenantbundne dual-read-regler.
  assert.equal(rules.split("child('legacyClaimsAllowlist').child(auth.uid).child('expiresAtMs').val() > now").length - 1, 115);
  ...
  assert.equal(rules.split("auth.token.perms.contains('|" ).length - 1, 192);
  ...
  // FLEETs serverejede serviceområder og de fælles ressourcekontrakter
  // løfter den til knap 495 kB. Loftet giver fortsat kun et lille spillerum
  // til bevidste regelændringer.
  ...
  // seks nye læseregler for serviceområder tilføjer tolv. De fire fælles
  // ressourceområder tilføjer yderligere otte revocation-kontroller.
  assert.equal(rules.split("child('authRevocations').child(auth.uid)").length - 1, 276);
