...
const root = resolve(".");
const out = join(root, "VEYRO_EJERKONSOL_REVIEW_V8_1_SUPPORT_AI.zip");
const report = join(root, "docs", "VEYRO_EJERKONSOL_REVIEW_V8_1_SUPPORT_AI.md");
const status = join(root, "docs", "VEYRO_EJERKONSOL_STATUS_V1.md");
const contract = join(root, "docs", "VEYRO_EJER_SUPPORT_KONTRAKT_INPUT_V1_1.md");
const screenshots = join(root, "docs", "screenshots", "ejer-review-v8-1-support-ai");
const verification = join(root, "docs", "VEYRO_EJERKONSOL_REVIEW_V8_1_SUPPORT_AI_ARCHIVE.json");
...
const stage = mkdtempSync(join(tmpdir(), "veyro-ejer-v8-1-support-"));
const unpacked = mkdtempSync(join(tmpdir(), "veyro-ejer-v8-1-support-check-"));
try {
  ...
  writeFileSync(join(stage, "archive-manifest.json"), `${JSON.stringify({
    version: "V8.1 Support-afstemning", finalHead, implementationCommit: "2c25c196ae980995849a12b06f805551c98f9f63", createdAt: new Date().toISOString(), entries,
  }, null, 2)}\n`);
  ...
  const result = {
    ...
    implementationCommit: "2c25c196ae980995849a12b06f805551c98f9f63",
