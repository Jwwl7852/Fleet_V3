...
const profiler = await dbGet("profiler") || {}; const alleProfiler = Object.values(profiler);
const dennis = alleProfiler.find((p) => /dennis/i.test(`${p?.navn} ${p?.email}`)); const joern = alleProfiler.find((p) => /j(ø|o)rn/i.test(`${p?.navn} ${p?.email}`));
...
assert.equal(Object.values(efterJoern.noter || {}).length, 1); assert.equal(Object.values(efterJoern.noter)[0].intern, true);
...
console.log(JSON.stringify({ ok: true, version: "V8 Support AI", E1_knownKnowledge: true, E2_unknownNoInventedFix: true, E3_staleRejected: true, E4_sameCaseDennisJoern: true, E5_internalExcludedAndNoMailjob: true, E6_privateDenied: true, externalAiCalled: false, mailSent: false }, null, 2));
