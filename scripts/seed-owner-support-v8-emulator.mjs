...
const profiler = Object.entries(await dbGet("profiler") || {}).map(([uid, profil]) => ({ ...(profil || {}), uid: profil?.uid || uid }));
const dennis = profiler.find((profil) => /dennis/i.test(`${profil?.navn || ""} ${profil?.email || ""}`)) || profiler.find((profil) => !/j(ø|o)rn/i.test(`${profil?.navn || ""} ${profil?.email || ""}`)) || profiler[0];
const joern = profiler.find((profil) => /j(ø|o)rn/i.test(`${profil?.navn || ""} ${profil?.email || ""}`));
...
const threads = {
  "v8-support-kendt": common("v8-support-kendt", {
    ...
    support: { nummer: "SUP-2026-0081", type: "adgang", status: "afventer_os", prioritet: "hoej", modul: "FLEET", version: "3.4.2", versionKilde: "Syntetisk V8.1-reviewfixture · ikke en verificeret produktudgivelse", fejltekst: "Session expired", forsoegt: "Browser genstartet; problemet fortsætter", ansvarligUid: dennis.uid, fristMs: now + 7_200_000 },
    beskeder: {
      m1: { id: "m1", provider: "fixture", retning: "indgaaende", fra: "maja@nordlys.syntetisk.invalid", til: "info@veyrosystems.com", emne: "Session udløber efter adgangsændring", tekst: marker("Efter en ændring i brugerens adgang får vi teksten 'Session expired' ved næste login. Vi har genstartet browseren. Hvad gør vi?"), sendtMs: now - 3_600_000 },
      m2: { id: "m2", provider: "fixture", retning: "udgaaende", fra: "info@veyrosystems.com", til: "maja@nordlys.syntetisk.invalid", emne: "Re: Session udløber efter adgangsændring", tekst: marker("Tak. Vi undersøger de registrerede oplysninger og vender tilbage med dokumenterede trin."), sendtMs: now - 3_000_000, sagsbehandlerNavn: "Jørn Testejer" },
      m3: { id: "m3", provider: "fixture", retning: "indgaaende", fra: "maja@nordlys.syntetisk.invalid", til: "info@veyrosystems.com", emne: "Re: Session udløber efter adgangsændring", tekst: marker("Problemet gælder kun én bruger efter en rolleændring. De øvrige brugere kan fortsat logge ind."), sendtMs: now - 2_400_000 },
      m4: { id: "m4", provider: "fixture", retning: "indgaaende", fra: "maja@nordlys.syntetisk.invalid", til: "info@veyrosystems.com", emne: "Re: Session udløber efter adgangsændring", tekst: marker("Vi har også prøvet et privat browservindue. Fejlteksten er stadig Session expired."), sendtMs: now - 1_900_000 },
    },
    noter: {
      n1: { id: "n1", tekst: marker("Bekræft versionsnummer og ændr ikke kundens roller under fejlsøgningen."), oprettetAf: joern.uid, oprettetMs: now - 1_800_000, intern: true },
      n2: { id: "n2", tekst: marker("Den seneste mailafsender er ikke automatisk den registrerede CRM-kontakt."), oprettetAf: dennis.uid, oprettetMs: now - 1_200_000, intern: true },
      n3: { id: "n3", tekst: marker("Brug kun kundegodkendt viden i det eksterne svar; intern forklaring er baggrund."), oprettetAf: joern.uid, oprettetMs: now - 600_000, intern: true },
    },
    sagsOplysninger: { version: { id: "version", label: "Kendt produktversion", vaerdi: "3.4.2", tilstand: "fixture", kilde: "Syntetisk V8.1-reviewfixture · ikke en verificeret produktudgivelse", raekke: 1 }, fejltekst: { id: "fejltekst", label: "Fejltekst", vaerdi: "Session expired", tilstand: "oplyst_af_kunden", kilde: "Kundens mail", raekke: 2 }, forsoegt: { id: "forsoegt", label: "Allerede forsøgt", vaerdi: "Browser genstartet", tilstand: "oplyst_af_kunden", kilde: "Kundens mail", raekke: 3 } },
  ...
  "v8-support-ukendt": common("v8-support-ukendt", {
    emne: "Support · Synkronisering stopper uden fejltekst", status: "afventer_os", ansvarligUid: joern.uid, kontaktNavn: "Søren Holm", kontaktEmail: "soeren@ukendt.syntetisk.invalid", virksomhedsnavn: "Vestby Service · syntetisk",
    support: { nummer: "SUP-2026-0082", type: "drift", status: "triage", prioritet: "normal", modul: "FLEET", version: "", fejltekst: "", forsoegt: "", ansvarligUid: joern.uid, fristMs: now + 21_600_000 },
