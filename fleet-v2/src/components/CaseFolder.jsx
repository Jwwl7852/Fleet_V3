...
export function CaseFolder({ caseId, onNavigate, dialog = false, onDirtyChange }) {
  ...
  return <Root className={`workspace-page case-folder-page${dialog ? " case-folder-dialog-content" : ""}`} {...(!dialog ? { id: "main-content" } : {})}>
    {!dialog ? <div className="profile-breadcrumb"><button onClick={() => onNavigate("/arbejdsko")} type="button">Arbejdskø</button><Icon name="chevron" size={13} /><span>{caseItem.reference}</span><em>Lokal prototype</em></div> : null}
