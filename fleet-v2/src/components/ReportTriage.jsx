...
export function ReportTriage({ reportId, initialViewState, onViewStateChange, onNavigate, onBack = onNavigate }) {
  ...
  return <main className={`workspace-page triage-page${mobileDetail ? " triage-detail-selected" : ""}`} id="main-content">
    <header className="page-heading-row"><div><h1>Indberetninger</h1><p>Gennemgå, vurder og forbind den oprindelige indberetning med samme sag i Arbejdskø.</p></div><button className="primary-button" type="button" onClick={() => onNavigate("/indberetninger/ny")}><Icon name="plus" size={17} />Ny indberetning</button></header>
    ...
    <ThreePanelWorkspace className="triage-layout" actorId={actor.id} tenantId={tenantId} screen="/fleet-v2/indberetninger">
      ...
      <section className="triage-detail-panel">{selectedReport && selectedCase ? <>
        ...
        <header><div><span className="eyebrow">{selectedReport.number} · {CASE_STATUSES[selectedCase.status]}</span><h2>{selectedReport.title}</h2><p>{unit ? `${unit.number} · ${modelLabel(unit)} · ${unit.department}` : `Enhedsrelation: ${selectedReport.unitId || selectedCase.unitId || "ikke oplyst"}`}</p>{selectedReport.origin === "service_automation" ? <span className="integration-badge">Automatisk oprettet fra Service</span> : null}</div><span className={`priority-pill ${selectedCase.priority}`}>{CASE_PRIORITIES[selectedCase.priority]} prioritet</span></header>
        {unit ? <div className="report-unit-summary"><UnitThumbnail unit={unit} /><div><strong>{unit.number}</strong><span>{modelLabel(unit)}</span><small>{formatMeter(unit)}</small></div><div className="inline-actions"><button type="button" onClick={() => onNavigate(`/enheder/${unit.id}`)}>Åbn enhed</button><button type="button" onClick={() => onNavigate(`/sager/${selectedCase.id}`)}>Sagsmappe <Icon name="external" size={13} /></button></div></div> : <div className="form-alert warning report-unit-missing" role="status"><span>Den tilknyttede enhed findes ikke i det aktuelle enhedsregister.</span><button type="button" onClick={() => onNavigate(`/sager/${selectedCase.id}`)}>Åbn sagsmappe</button></div>}
