...
export function WorkshopTaskCreator({ caseItem, report, relations, onCreate, onClose, onNavigate }) {
  ...
  const workshops = selectableWorkshops(relations.workshops);
  const initialValues = {
  ...
  const submit = async (event) => {
    ...
    if (!confirmBusinessSave("Opret værkstedsopgaven?")) return;
  ...
  if (existing) return <div className="modal-backdrop" role="presentation" onMouseDown={onBackdropMouseDown}><section ref={dialogRef} tabIndex={-1} className="workshop-dialog compact" role="dialog" aria-modal="true" aria-labelledby="existing-workshop-title"><button className="dialog-close" onClick={requestClose} type="button" aria-label="Luk"><Icon name="close" /></button><h2 id="existing-workshop-title">Aktiv værkstedsopgave findes</h2><p>Sagen er allerede knyttet til {existing.number}. Der oprettes ikke en dublet.</p><button className="primary-button full" type="button" onClick={() => onNavigate(`/vaerksted/${existing.id}`)}>Åbn {existing.number}</button></section></div>;
  return <div className="modal-backdrop" role="presentation" onMouseDown={onBackdropMouseDown}><form ref={dialogRef} tabIndex={-1} className="workshop-dialog" role="dialog" aria-modal="true" aria-labelledby="create-workshop-title" onSubmit={submit}>
    <header><div><span className="eyebrow">{caseItem.number} · lokal prototype</span><h2 id="create-workshop-title">Opret værkstedsopgave</h2><p>Oplysninger og billeder følger automatisk den eksisterende sag.</p></div><button className="dialog-close" onClick={requestClose} type="button" aria-label="Luk"><Icon name="close" /></button></header>
    <div className="workshop-form-grid">
      ...
      <label>Værksted<select aria-label="Vælg værksted" value={values.workshopId} onChange={(event) => set("workshopId", event.target.value)}>{workshops.map((item) => <option value={item.id} key={item.id}>{item.name} · {item.kind === "internal" ? "Internt" : "Eksternt"}</option>)}</select></label>
    ...
    <footer><button className="secondary-button" type="button" onClick={requestClose}>Annuller</button><button className="primary-button" type="submit" disabled={busy}><Icon name="plus" size={16} />{busy ? "Opretter …" : "Opret opgave"}</button></footer>
  </form></div>;
}
