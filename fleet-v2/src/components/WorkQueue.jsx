...
export function WorkQueue({ caseId, initialViewState, onViewStateChange, onNavigate, onBack = onNavigate }) {
  ...
  return <main className="workspace-page queue-page" id="main-content">
    ...
    <section className="queue-actions-toolbar" aria-label="Handlinger og visning"><button className="secondary-button" type="button" onClick={() => setCreatingManual(true)}><Icon name="plus" size={15} />Manuel sag</button><div className="view-switch"><button aria-pressed={view === "kanban"} onClick={() => setView("kanban")} type="button"><Icon name="grid" size={16} />Kanban</button><button aria-pressed={view === "table"} onClick={() => setView("table")} type="button"><Icon name="table" size={16} />Tabel</button></div></section>
