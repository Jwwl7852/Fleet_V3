...
export function CaseStatusConfirmDialog({ caseItem, busy = false, onCancel, onConfirm }) {
  const { dialogRef, requestClose, onBackdropMouseDown } = useModalDialog({ onClose: onCancel, busy });
  if (!caseItem) return null;
  return <div className="modal-layer case-status-confirm-layer" role="presentation" onMouseDown={onBackdropMouseDown}>
    <section ref={dialogRef} tabIndex={-1} className="modal-card case-status-confirm" role="alertdialog" aria-modal="true" aria-labelledby="back-to-new-title" aria-describedby="back-to-new-description">
      ...
      <footer><button className="secondary-button" type="button" onClick={requestClose} disabled={busy}>Annuller</button><button className="primary-button" type="button" onClick={onConfirm} disabled={busy}>{busy ? "Flytter …" : "Ja, flyt tilbage"}</button></footer>
    </section>
  </div>;
}
