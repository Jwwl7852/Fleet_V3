...
function LiveOverview({ units, resourceOptions, storedPositions, filters, setFilters, selectedUnitId, setSelectedUnitId, setMode, setMobileView, mobileView }) {
  const [listOpen, setListOpen] = useState(true); const [focusUnitId, setFocusUnitId] = useState(null); const now = useMemo(() => new Date().toISOString(), []);
...
function Playback({ packets, selectedIndex, setSelectedIndex, playing, setPlaying, speed, setSpeed }) {
  return <div className="history-playback"><button className="history-play" type="button" aria-label={playing ? "Pause" : "Afspil tur"} onClick={() => setPlaying((value) => !value)}>{playing ? "Ⅱ" : <Icon name="play" size={18} />}</button><select aria-label="Afspilningshastighed" value={speed} onChange={(event) => setSpeed(Number(event.target.value))}><option value="1">1×</option><option value="2">2×</option><option value="4">4×</option></select><span>{localTime(packets[0]?.measuredAt)}</span><input aria-label="Valgt tidspunkt i turen" type="range" min="0" max={Math.max(0, packets.length - 1)} value={Math.min(selectedIndex, Math.max(0, packets.length - 1))} onChange={(event) => { setPlaying(false); setSelectedIndex(Number(event.target.value)); }} /><span>{localTime(packets.at(-1)?.measuredAt)}</span><output>{localTime(packets[selectedIndex]?.measuredAt, true)}</output></div>;
}
...
function MetricChart({ title, unit, values, colorClass }) {
  ...
  const points = valid.map((item, index) => `${valid.length === 1 ? 0 : index / (valid.length - 1) * 100},${42 - Number(item.value) / max * 36}`).join(" ");
  return <article className="metric-chart"><header><span><strong>{title}</strong><small>{unit}</small></span><b>{valid.at(-1)?.value ?? "–"} {valid.length ? unit : ""}</b></header><svg viewBox="0 0 100 46" preserveAspectRatio="none" aria-label={`${title} over tid`}><line x1="0" y1="42" x2="100" y2="42" /><polyline className={colorClass} points={points} /></svg>{valid.length ? <small>{localTime(valid[0].measuredAt)} – {localTime(valid.at(-1).measuredAt)} · huller forbindes ikke i providerdata</small> : <p>Ingen understøttede målinger i perioden.</p>}</article>;
}
