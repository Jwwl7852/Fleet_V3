...
export function LiveMap({ initialViewState, onNavigate, onViewStateChange }) {
  ...
  return <main className={`workspace-page live-map-page mobile-${mobileView}`} id="main-content">
    <header className="workspace-heading live-map-title"><div><h1>Livekort</h1><p>Se senest kendte position og aktualitet for flådens enheder.</p></div><div><span className="live-demo-badge">Syntetiske positioner – ikke live</span><button className="secondary-button" type="button" onClick={() => setDemoOpen((value) => !value)}><Icon name="service" size={16} />Demokontrol</button></div></header>
