...
function DocumentDetail({ document, dataset, units, relations, onBack, onNavigate: _onNavigate, actions }) {
  ...
  if (!document) return <section className="document-not-found"><Icon name="warning" size={34} /><h2>Dokumentet findes ikke</h2><p>ID’et findes ikke i det lokale dokumentregister.</p><button className="secondary-button" type="button" onClick={() => onBack("/dokumenter")}>Til dokumentoversigten</button></section>;
...
export function DocumentsOverview({ documentId, onNavigate, onBack = onNavigate }) {
