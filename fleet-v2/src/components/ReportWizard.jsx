...
import { DEFAULT_FLEET_CATEGORIES, categoriesForPurpose, categoryById } from "../data/fleetCategories";
...
const initial = { unitId: "", type: "fault", categoryId: "", category: "", severity: "moderate", title: "", description: "", images: [], media: [], meter: "", usability: "uncertain", incident: emptyIncident, draftId: null, reference: null };

function validate(step, values) {
  const errors = {};
  if (step === 1 && !values.unitId) errors.unitId = "Vælg en enhed.";
  if (step === 2) {
    if (!values.categoryId) errors.categoryId = "Vælg en kategori.";
...
export function ReportWizard({ onNavigate, imageProcessor = prepareUnitImage, reporter = demoReporter, allowedUnitIds = null, mobile = false }) {
  ...
  const reportCategories = useMemo(() => categoriesForPurpose(relations.fleetCategories || DEFAULT_FLEET_CATEGORIES, "report", values.categoryId), [relations.fleetCategories, values.categoryId]);
  ...
  useEffect(() => {
    ...
    if (draft && (!allowedUnitIds || !draft.unitId || allowedUnitIds.includes(draft.unitId))) {
      const matchedCategory = draft.categoryId ? categoryById(reportCategories, draft.categoryId) : reportCategories.find((item) => item.name === draft.category);
      setValues({ ...initial, ...draft, categoryId: matchedCategory?.id || "", category: matchedCategory?.name || draft.category || "", incident: { ...emptyIncident, ...(draft.incident || {}) } });
      setStep(draft.savedStep || 1);
    }
    else if (draft && mobile) setMessage("Kladden tilhører en anden demo-bruger eller en enhed, du ikke længere har adgang til.");
  }, [draftId, relations.reportDrafts, mobile, reporter.id, allowedUnitIds, reportCategories]);
  ...
  const submit = async () => {
    ...
    if (Object.keys(all).length) { setErrors(all); setStep(all.unitId ? 1 : all.categoryId || all.title || all.description || all.damageType ? 2 : 3); return; }
  ...
  const cancel = () => {
    const changed = values.unitId || values.categoryId || values.title || values.description || values.images.length || values.media.length;
