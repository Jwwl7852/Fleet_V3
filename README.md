...
| **[BESLUTNINGER.md](BESLUTNINGER.md)** | De 123 beslutninger med begrundelser. Læs den før du bryder med noget |
...
| 123 | **Individuel medarbejder-rettighedsoverstyring — oven på rollen, ikke i stedet for den.** Produktejerens krav: "helt ned på medarbejder niveau bestemme hvad de kan se og har rettigheder til." `permsForBruger(rolle, roller, override)` lægger et `{tilfoejet, fjernet}`-delta oven på `permsForTenant()`; ny funktion `medarbejderrettighederskriv` skriver `brugere/<uid>/permsOverride` og minter claims for KUN den ene bruger (ikke en løkke over en hel rolle). ⚠ **Admin kan hverken indskrænkes eller udvides via override** — samme invariant som beslutning 121, ét lag dybere. ⚠ **`laaserUdeMedarbejder()` udvider NOEGLEPERM-beskyttelsen til personer**: kan ikke fjerne `brugere.skriv` fra sin egen adgang, og ikke fra den sidste PERSON der reelt har den (regnet på tværs af alle brugeres effektive perms, ikke kun rollerne). ⚠ **Fandt og rettede en cascade-fejl i `rolleskriv`** undervejs: den beregnede ét fælles claim-udtryk FØR løkken over alle med rollen — det ville have overskrevet enhver medarbejders individuelle override ved hver rolleredigering. ⚠ **`permsOverride` er samme figur som `roller/`**: en KILDE, aldrig et håndhævelsespunkt — ingen regel i `firebase.rules.json` slår nogensinde op i feltet. UI genbruger `Rolleeditor`s mønster og den allerede eksisterende `.fc-kontakt`-slider fra `ProcureGodkendelsesregler.jsx`, rettet mod én bruger i stedet for én rolle | `permissions.js`, `functions/index.js`, `firebase.rules.json`, `brugere.js`, `Brugere.jsx` |
