...
export default function RessourceKatalogOpsaetning({
  ...
  return <div className="fc-grid" style={{ gap: 16 }}>
    ...
    <Kort titel={`${titel} – kundedefinerede kategorier`} handling={<Knap onClick={() => setKategori(tomKategori)}>Ny kategori</Knap>}>
      <p className="fc-hint">Kategorier har stabile ID'er og deaktiveres i stedet for at blive slettet. Eksisterende ressourcer og historik beholder derfor deres reference.</p>
      <div className="fc-tabel-wrap"><table className="fc-tabel"><thead><tr><th>Kategori</th><th>Sortering</th><th>Status</th><th>Handling</th></tr></thead><tbody>
        {kategorier.data.map((post) => <tr key={post.id}><td><b>{post.navn}</b><br /><small>{post.id}</small></td><td>{post.sortering}</td><td><Pille tone={post.aktiv === false ? "neutral" : "ok"}>{post.aktiv === false ? "Inaktiv" : "Aktiv"}</Pille></td><td><span className="fc-med-ikon"><Knap onClick={() => setKategori(post)}>Redigér</Knap><Knap disabled={gemmer} onClick={() => skiftKategori(post)}>{post.aktiv === false ? "Genaktivér" : "Deaktivér"}</Knap></span></td></tr>)}
        {!kategorier.data.length ? <tr><td colSpan="4">Ingen kategorier er oprettet endnu.</td></tr> : null}
      </tbody></table></div>
    </Kort>
    <Kort titel={kategori.id ? `Redigér ${kategori.navn}` : "Opret kategori"}>
      <form className="fc-form-grid" onSubmit={gemKategori}><label className="fc-felt"><span>Navn</span><input value={kategori.navn} maxLength="80" onChange={(e) => setKategori({ ...kategori, navn: e.target.value })} /></label><label className="fc-felt"><span>Sortering</span><input type="number" min="0" max="9999" value={kategori.sortering} onChange={(e) => setKategori({ ...kategori, sortering: e.target.value })} /></label><div><Knap type="submit" disabled={gemmer}>{gemmer ? "Gemmer …" : "Gem kategori"}</Knap></div></form>
    </Kort>
    </> : null}
    {hardwareArt ? <>
      <Kort titel={`${hardwareArt.toUpperCase()}-hardware`} handling={<Knap onClick={() => setEnhed(tomHardware)}>Registrér hardware</Knap>}>
        <p className="fc-hint">Kun interne hardwareoplysninger registreres. Ingen ekstern tjeneste aktiveres. Tilknytninger ændres atomisk fra ressourceformularen.</p>
        <div className="fc-tabel-wrap"><table className="fc-tabel"><thead><tr><th>Serienummer</th><th>Model</th><th>Status</th><th>Tilknytning</th><th>Handling</th></tr></thead><tbody>
          {hardware.data.map((post) => <tr key={post.id}><td><b>{post.serienummer}</b><br /><small>{post.id}</small></td><td>{[post.leverandoer, post.model].filter(Boolean).join(" · ") || "—"}</td><td><Pille tone={post.status === "aktiv" ? "ok" : "neutral"}>{post.status === "aktiv" ? "Aktiv" : "Inaktiv"}</Pille></td><td>{post.tilknytning?.ressourceId || "Ledig"}</td><td><Knap onClick={() => setEnhed(post)}>Redigér</Knap></td></tr>)}
          {!hardware.data.length ? <tr><td colSpan="5">Ingen {hardwareArt.toUpperCase()}-enheder er registreret.</td></tr> : null}
        </tbody></table></div>
      </Kort>
