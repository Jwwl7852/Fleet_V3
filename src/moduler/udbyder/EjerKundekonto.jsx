...
const oereTilKrInput = (oere) => oere === "" || oere == null ? "" : String(Number(oere) / 100).replace(".", ",");
const krInputTilOere = (kroner) => kroner === "" || kroner == null ? "" : Math.round(Number(String(kroner).replace(",", ".")) * 100);
...
export default function EjerKundekonto() {
  ...
  return <div className="ejer-konto-side">
    ...
    {fane === "forbrug" && <section className="ejer-konto-kort"><h2>Brugere og enheder</h2><p>Aftalt antal, målt forbrug og servermæssigt håndhævet grænse er tre forskellige oplysninger.</p>{antalFelt("medarbejderbrugere", "Brugere — administrative", maaling?.brugere?.desktop)}{antalFelt("chauffoerbrugere", "Brugere — operative", maaling?.brugere?.chauffoer)}{antalFelt("enheder", "Aftalte enheder", maaling?.koeretoejer)}<div className="ejer-konto-info"><b>{maaling?.brugere?.unikkePersoner != null ? `${maaling.brugere.unikkePersoner} unikke personer · ${maaling.brugere.administratorer ?? "Ukendt antal"} administratorer` : "Personer og rettigheder tælles hver for sig"}</b><p>En person med adgang til flere moduler tæller kun én gang. Administratorinvitationer er adgang til kundens administration og er ikke ekstra brugerlicenser.</p></div></section>}
    {fane === "obd" && <section className="ejer-konto-kort"><h2>OBD-hardware og data</h2><p>Hardware er et engangsbeløb; SIM/data er en løbende tjeneste. Antal enheder i flåden, købte OBD-enheder og aktive forbindelser er ikke det samme.</p><div className="ejer-konto-formgrid"><Felt label="Aftalt OBD-hardware" type="number" min="0" value={form.obd.hardwareAntal} disabled={accepteret} onChange={(v) => sæt("obd", "hardwareAntal", v)} hint="Købte fysiske enheder"/><Felt label="Hardwarepris pr. stk. (kr.)" type="number" min="0" step="0.01" value={oereTilKrInput(form.obd.hardwarePrisOere)} disabled={accepteret} onChange={(v) => sæt("obd", "hardwarePrisOere", krInputTilOere(v))} hint="Engangspris pr. OBD-enhed"/><Felt label="Aftalte dataabonnementer" type="number" min="0" value={form.obd.dataabonnementAntal} disabled={accepteret} onChange={(v) => sæt("obd", "dataabonnementAntal", v)}/><Felt label="Dataabonnement pr. måned (kr.)" type="number" min="0" step="0.01" value={oereTilKrInput(form.obd.dataabonnementPrisOere)} disabled={accepteret} onChange={(v) => sæt("obd", "dataabonnementPrisOere", krInputTilOere(v))} hint="Løbende tjeneste, adskilt fra hardware"/><Felt label="Registreret leveret antal" type="number" min="0" value={form.obd.leveretAntal} onChange={(v) => sæt("obd", "leveretAntal", v)} hint="Manuelt registreret; ikke telemetri"/><Felt label="Registreret tilknyttet antal" type="number" min="0" value={form.obd.tilknyttetAntal} onChange={(v) => sæt("obd", "tilknyttetAntal", v)} hint="Manuelt registreret; siger ikke online"/></div><div className="ejer-konto-info"><b>Aktive OBD-forbindelser: Ikke tilgængeligt</b><p>Der er ingen tilsluttet telemetrikilde. Konsollen viser derfor hverken online-status eller nul aktive forbindelser.</p></div></section>}
    ...
    */}
    ...
    {fane === "administratorer" && <Administratorer data={data} genindlaes={genindlaes}/>}
    {fane === "historik" && <Historik data={data}/>}
    {!["administratorer", "historik"].includes(fane) && <footer className="ejer-konto-gem"><div><b>{dirty ? "Ikke-gemte ændringer" : "Kontoen er ajour"}</b><span>{aendringer.length ? aendringer.join(" · ") : "Ingen kommercielle forskelle i denne version."}</span><small>Status efter gem uden aktivering: Opsætning mangler.</small></div><div><Knap disabled={arbejder || !dirty} onClick={() => gem(false, false)}>Gem og færdiggør senere</Knap><Knap variant="primaer" disabled={arbejder || !dirty} onClick={() => gem(true, true)}>{form.abonnement.virkningsdato > isoIdag() ? "Planlæg ændring" : "Kontrollér og aktivér"}</Knap></div></footer>}
