...
function Tilbudsformular({ aktuel, virksomheder, prisliste, onGemt, onAnnuller }) {
  ...
  const foreslaa = async (felt, spoergsmaal) => {
    ...
    try {
      ...
      if (!traad || platform.integrationer?.openai?.status !== "aktiv") {
        ...
        const tilpasning = aiInstruks.trim() ? ` Sælgerens anvisning er indarbejdet: ${aiInstruks.trim()}` : "";
        const lokale = {
          indledning: `Tak for den gode dialog. Dette forslag samler ${kunde}s bekræftede behov i et konkret, gennemgåeligt oplæg.${tilpasning}`,
          behovstekst: `${kunde} har behov for et samlet overblik og en tydelig opstart. Omfang, startdato og eventuelle integrationer skal bekræftes før aftale.${tilpasning}`,
          loesningsbeskrivelse: `Veyro samler de valgte moduler i én løsning. Implementering, ansvar og leverancer aftales i en fælles opstartsplan.${tilpasning}`,
        };
  ...
  return <div className="fc-grid ejer-tilbudsredigering">
    ...
    <Kort titel="Kundetekst og Veyro-salgsassistent">
      ...
      {aiForslag && <div className="ejer-ai-resultat"><strong>AI-forslag · gennemgå før indsættelse</strong><p>{aiForslag.tekst}</p><Knap variant="primaer" onClick={() => { setF((x) => ({ ...x, [aiForslag.felt]: aiForslag.tekst })); setAiForslag(null); }}>Indsæt i tilbud</Knap><Knap onClick={() => foreslaa(aiForslag.felt, `Revidér forslaget efter sælgerens anvisning: ${aiInstruks || "gør teksten mere konkret"}`)} disabled={gemmer}>Lav revideret forslag</Knap><Knap onClick={() => setAiForslag(null)}>Forkast</Knap></div>}
    </Kort>
    <Kort titel="Rateblad">
