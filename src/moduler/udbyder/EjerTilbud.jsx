...
function Tilbudsformular({ aktuel, virksomheder, prisliste, onGemt, onAnnuller }) {
  ...
  return <div className="fc-grid ejer-tilbudsredigering">
    <header className="ejer-tilbud-redigerhoved"><div><small>Tilbudskladde · {aktuel ? `næste version ${Number(aktuel.aktuelVersion || 0) + 1}` : "nyt tilbud"}</small><h2>{aktuel?.nummer || "Nyt tilbud"} · {kundeNavn}</h2><span>{aktuel?.accept ? "Separat kladde — accepteret version er fortsat låst" : "Kladde · ikke sendt"}</span></div><div><small>Rateblad</small><b>{f.prislisteId || prisliste?.id || "Ikke valgt"}</b></div></header>
    ...
    {aktivFane==="tekst"&&<Kort titel="Kundetekst og Veyro-salgsassistent">
      ...
      <Felt id="tilbud-ai-instruks" label="Sælgerens anvisning til næste forslag" vaerdi={aiInstruks} saet={setAiInstruks} multiline hint="Fx: Gør teksten kortere og fremhæv pilotens afgrænsning. Anvisningen sendes ikke til kunden." />
      ...
      {aiForslag && <div className="ejer-ai-resultat"><strong>AI-forslag · gennemgå før indsættelse</strong><p>{aiForslag.tekst}</p>{f[aiForslag.felt]!==aiForslag.basisTekst&&<p className="ejer-advarsel">Feltet er ændret efter forslaget blev lavet. Lav et nyt forslag for at undgå at overskrive nyere tekst.</p>}<Knap variant="primaer" disabled={f[aiForslag.felt]!==aiForslag.basisTekst} onClick={() => { setF((x) => ({ ...x, [aiForslag.felt]: aiForslag.tekst })); setAiForslag(null); }}>Indsæt i tilbud</Knap><Knap onClick={() => foreslaa(aiForslag.felt, `Revidér forslaget efter sælgerens anvisning: ${aiInstruks || "gør teksten mere konkret"}`)} disabled={gemmer}>Lav revideret forslag</Knap><Knap onClick={() => setAiForslag(null)}>Forkast</Knap></div>}
    </Kort>}
