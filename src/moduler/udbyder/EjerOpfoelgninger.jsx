...
export default function EjerOpfoelgninger() {
  const [data, setData] = useState(null); const [valgt, setValgt] = useState(""); const [fane, setFane] = useState("klar");
  const [arbejder, setArbejder] = useState(false); const [besked, setBesked] = useState(""); const [udsaetTil, setUdsaetTil] = useState("");
  ...
  const viste = grupper[fane] || []; const aktuel = poster.find((o) => o.id === valgt) || viste[0];
  useEffect(() => { if (viste.length && !viste.some((x)=>x.id===valgt)) setValgt(viste[0].id); }, [fane, data]);
