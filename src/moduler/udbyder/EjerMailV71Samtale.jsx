...
export default function EjerMailV71Samtale({ valgt, bruger, mailsignatur, hent, params, lokaleKladder, setLokaleKladder, tilbage, supportVisning = false }) {
  ...
  const foreslaaSvar = async ({ blivPaaFane = false } = {}) => {
    ...
    if (supportVisning) {
      const r = await udfoer(() => gemSupportAiForslag({
  ...
  return <div className="ejer-mail-v7-detalje">
    <div className="ejer-mail-detaljetop"><button type="button" className="ejer-tilbage" onClick={() => tilbage(beskidt)}>← Tilbage til {supportVisning ? "supportkø" : params.get("fra") === "sager" ? "sager og mapper" : "indbakke"}</button><Testmaerke/></div>
    <header className="ejer-mail-sagshoved"><div><small>{supportVisning ? `${valgt.support?.nummer || "Supportsag"} · ` : `${SAGSTYPE_LABEL[valgt.sagstype] || "Kræver gennemgang"} · `}<Kildetekst traad={valgt} bruger={bruger}/></small><h2>{valgt.emne}</h2><div className="ejer-mail-chips"><span>{modpart}</span>{supportVisning && <span>Modul: {valgt.support?.modul || "Ukendt"}</span>}<span>Ansvarlig: {ejerNavn(valgt.ansvarligUid, bruger)}</span><span>{supportVisning ? (valgt.support?.status || "ny").replaceAll("_", " ") : statusTekst[valgt.status] || valgt.status}</span><span>{valgt.delingsstatus === "delt" ? "Delt med Dennis og Jørn" : "Personlig/afventer deling"}</span></div></div><div className="fc-actions"><button type="button" className="fc-btn" onClick={overtag} disabled={arbejder}>Overtag sag</button>{!internSag && valgt.links?.virksomhedId && <Link className="fc-btn" to="/main/kunder">Åbn kundekort</Link>}{supportVisning ? <Link className="fc-btn" to={`/main/mail/indbakker?postkasse=faelles&sag=${encodeURIComponent(valgt.id)}`}>Åbn i Mail</Link> : !internSag && <button type="button" className="fc-btn" onClick={supportHandling}>{valgt.sagstype === "support" ? "Åbn Support" : "Knyt til Support"}</button>}</div></header>
