...
import { supportKildevisning, supportSagFakta } from "../../fleet/ejer-support-ai.js";
...
export default function EjerMailV71Samtale({ valgt, bruger, mailsignatur, hent, params, lokaleKladder, setLokaleKladder, tilbage, supportVisning = false, supportAdapter = null, supportKontroller = null }) {
  ...
  const svarScrollRef = useRef(null); const svarEditorRef = useRef(null);
  ...
  const supportFakta = supportSagFakta(valgt); const supportKilder = supportKildevisning(valgt);
  ...
  useEffect(() => {
    if (aktivFane !== "svarudkast") return;
    const id = requestAnimationFrame(() => {
      const rullefelt = svarScrollRef.current; const editor = svarEditorRef.current;
      if (rullefelt && editor) rullefelt.scrollTop = Math.max(0, editor.offsetTop - rullefelt.offsetTop - 8);
    });
    return () => cancelAnimationFrame(id);
  }, [aktivFane, mobilpanel, valgt.id]);
  ...
  const erSupport = valgt.sagstype === "support";
  const adapterFelt = erSupport ? { kildeAdapter: valgt.kilde?.adapter || "" } : {};
  ...
  const overtag = () => udfoer(() => (erSupport && supportAdapter ? supportAdapter.overtag : opdaterSalgstraad)({ traadId: valgt.id, status: valgt.status, ansvarligUid: bruger.uid, forventetRevision: valgt.revision, ...adapterFelt, ...(valgt.links || {}) }), "Sagen er overtaget af " + egetNavn(bruger) + ".");
  const gemNote = () => internNote.trim() && udfoer(async () => { const r = await (erSupport && supportAdapter ? supportAdapter.noteSkriv : opretSalgsnote)({ traadId: valgt.id, tekst: internNote, ...adapterFelt }); if (r.ok) setInternNote(""); return r; }, "Den interne note er gemt og sendes aldrig til kunden.");
  ...
  const gemSvar = async () => {
    const r = await udfoer(() => (erSupport && supportAdapter ? supportAdapter.kladdeGem : gemKommunikationssvarkladde)({
      ...
      forventetSagRevision: valgt.revision, forventetRevision: kladde?.revision || 0, ...adapterFelt,
  ...
  const godkendSvar = async () => {
    if (!kladde) return;
    const r = await udfoer(() => (erSupport && supportAdapter ? supportAdapter.svarGodkend : godkendKommunikationssvar)({ traadId: valgt.id, id: kladde.id, forventetRevision: kladde.revision, ...adapterFelt }), "Det viste svar er godkendt og afventer særskilt afsendelse.");
  ...
  const supportStatus = supportStatusVisning(valgt.support?.status, valgt.ansvarligUid || valgt.support?.ansvarligUid);
  const sekundæreHandlinger = <><button type="button" className="fc-btn" onClick={overtag} disabled={arbejder}>Overtag sag</button>{!internSag && valgt.links?.virksomhedId && <Link className="fc-btn" to="/main/kunder">Åbn kundekort</Link>}{supportVisning ? <Link className="fc-btn" to={`/main/mail/indbakker?postkasse=faelles&sag=${encodeURIComponent(valgt.id)}`}>Åbn i Mail</Link> : !internSag && <button type="button" className="fc-btn" onClick={supportHandling}>{valgt.sagstype === "support" ? "Åbn Support" : "Knyt til Support"}</button>}</>;

  return <div className={`ejer-mail-v7-detalje ${supportVisning ? "ejer-support-detalje" : ""}`}>
    {!supportVisning && <div className="ejer-mail-detaljetop"><button type="button" className="ejer-tilbage" onClick={() => tilbage(beskidt)}>← Tilbage til {params.get("fra") === "sager" ? "sager og mapper" : "indbakke"}</button><Testmaerke/></div>}
    <header className={`ejer-mail-sagshoved ${supportVisning ? "ejer-support-kompakthoved" : ""}`}><div>{supportVisning && <div className="ejer-support-hovedmeta"><button type="button" className="ejer-tilbage" onClick={() => tilbage(beskidt)}>← Tilbage til supportkø</button><Testmaerke/></div>}<small>{supportVisning ? `${valgt.support?.nummer || "Supportsag"} · ` : `${SAGSTYPE_LABEL[valgt.sagstype] || "Kræver gennemgang"} · `}<Kildetekst traad={valgt} bruger={bruger}/></small><h2>{valgt.emne}</h2><div className="ejer-mail-chips"><span>{modpart}</span>{supportVisning && <><span>Prioritet: {supportKontroller?.prioritetLabel || "Normal"}</span><span>Modul: {valgt.support?.modul || "Ukendt"}</span></>}<span>Ansvarlig: {ejerNavn(valgt.ansvarligUid || valgt.support?.ansvarligUid, bruger)}</span><span>{valgt.sagstype === "support" ? supportStatus.label : statusTekst[valgt.status] || valgt.status}</span>{supportVisning && <span>Kanal: {supportAdapter?.kanalFor?.(valgt) === "portal" ? "Kundeportal" : "E-mail"}</span>}<span>{valgt.delingsstatus === "delt" ? "Delt med Dennis og Jørn" : "Personlig/afventer deling"}</span></div></div><div className="ejer-support-hovedhandlinger"><div className="ejer-support-statushandlinger">{supportVisning && <><button type="button" className="fc-btn" disabled={supportKontroller?.arbejder} onClick={() => supportKontroller?.skiftStatus("afventer_os")}>Afventer os</button><button type="button" className="fc-btn" disabled={supportKontroller?.arbejder} onClick={() => supportKontroller?.skiftStatus("afventer_kunden")}>Afventer kunden</button><button type="button" className="fc-btn fc-btn-primary" disabled={supportKontroller?.arbejder} onClick={() => supportKontroller?.skiftStatus("loest")}>Markér løst</button></>}</div><div className="fc-actions ejer-support-desktophandlinger">{sekundæreHandlinger}</div>{supportVisning && <details className="ejer-support-mobilmenu"><summary className="fc-btn">Flere handlinger</summary><div>{sekundæreHandlinger}</div></details>}</div></header>
    {supportKontroller?.besked && <p className="ejer-handlingssvar ejer-support-kompaktbesked" role="status">{supportKontroller.besked}</p>}
      ...
      <aside id="mobilpanel-svar" aria-labelledby="mobilfane-svar" className="ejer-design-kort ejer-mail-svarfokus"><header><span className="ejer-ikonfelt"><EjerIkon navn="sparkles"/></span><div><h2>Svar og AI</h2><small>Fælles internt arbejdsrum · eksterne tjenester ikke tilsluttet</small></div></header>
        ...
        <div className="ejer-mail-ai-paneler">
          {aktivFane === "svarudkast" && <section id="panel-svarudkast" role="tabpanel" aria-labelledby="fane-svarudkast" className="ejer-mail-ai-panel ejer-svarudkastpanel">
            <div ref={svarScrollRef} className="ejer-mail-ai-scrollregion" tabIndex="0" aria-label="Svarudkastets indhold">
            ...
            <div ref={svarEditorRef} className="ejer-mail-editor"><div className="ejer-mail-formatlinje" aria-label="Tekstformatering"><button type="button" aria-label="Fed">B</button><button type="button" aria-label="Kursiv"><i>I</i></button><button type="button" aria-label="Understreget"><u>U</u></button><span/><button type="button" aria-label="Punktopstilling">☷</button></div><textarea aria-label="Svarudkast" value={svartekst} onChange={(event) => setLokaleKladder((gammel) => ({ ...gammel, [valgt.id]: event.target.value }))} rows="12"/></div>
          ...
          {aktivFane === "ai-chat" && <section id="panel-ai-chat" role="tabpanel" aria-labelledby="fane-ai-chat" className="ejer-mail-ai-panel ejer-ai-chatpanel"><div className="ejer-ai-chatintro"><EjerIkon navn="info" size={18}/><span>Kun Dennis og Jørn kan se samtalen. Den kan aldrig medsendes til kunden.</span></div><div className="ejer-ai-chathistorik" tabIndex="0" aria-label="Fælles AI-chathistorik">{aiUdvekslinger.map((gruppe) => <article key={gruppe.id} className="ejer-ai-udveksling"><section className="ejer"><header><b>{gruppe.ejerpost.aktorNavn}</b><time>{dato(gruppe.ejerpost.oprettetMs)}</time></header><p>{gruppe.ejerpost.tekst}</p></section>{gruppe.aipost && <section className="ai"><header><b>{supportVisning ? "Lokal Support-AI" : "Lokal AI-testadapter"}</b><time>{dato(gruppe.aipost.oprettetMs)}</time></header><p>{gruppe.aipost.tekst}</p></section>}</article>)}{!aiUdvekslinger.length && <p className="ejer-tomlinje">Ingen intern AI-chat endnu. Skriv den første instruktion nedenfor.</p>}{aiForslag && <section className={"ejer-ai-forslag " + (forslagForældet ? "foraeldet" : "")}><header><b>Forslag før indsættelse</b><span>{forslagForældet ? "Forældet grundlag" : "Ikke indsat"}</span></header><p>{aiForslag.tekst}</p><button type="button" className="fc-btn fc-btn-primary" onClick={indsætForslag} disabled={forslagForældet}>Indsæt i svarudkast</button></section>}</div><section className="ejer-mail-aiinstruks" data-testid="fast-ai-chat-komposer"><label>Fortsæt den fælles AI-chat<textarea rows="4" value={aiInstruks} onInput={voks} onChange={(event) => setAiInstruks(event.target.value)} placeholder={supportVisning ? "Fx: Opsummér problemet, find dokumenteret løsning og foreslå næste spørgsmål" : "Fx: Gør forslaget kortere, men behold alle fakta"}/></label><button type="button" className="fc-btn fc-btn-primary" onClick={foreslaaSvar} disabled={arbejder || !aiInstruks.trim()}>{supportVisning ? "Fejlsøg og foreslå svar" : "Lav nyt forslag"}</button><small>Enter laver ny linje · indholdet er kun internt</small></section><p className="fc-hint">Forslaget dannes af en deterministisk lokal testadapter. OpenAI kaldes ikke.</p></section>}
          {aktivFane === "oplysninger" && <section id="panel-oplysninger" role="tabpanel" aria-labelledby="fane-oplysninger" className="ejer-mail-ai-panel">{supportVisning && <><div className="ejer-support-fakta"><article><b>Kunde</b><span>{supportFakta.kunde}</span><small>Kilde: {supportFakta.kundeKilde}</small></article><article><b>Registreret CRM-kontakt</b><span>{supportFakta.kontaktNavn}</span><small>{supportFakta.kontakt} · Kilde: {supportFakta.kontaktKilde}</small></article><article><b>Mailafsender</b><span>{supportFakta.afsender}</span><small>Kilde: {supportFakta.afsenderKilde}</small></article><article><b>Modul og registreret version</b><span>{supportFakta.modul} · {supportFakta.version}{supportFakta.versionErSyntetisk ? " · testværdi" : ""}</span><small>Kilde: {supportFakta.versionKilde}</small></article><article><b>Problem</b><span>{supportFakta.problem}</span></article><article><b>Allerede forsøgt</b><span>{supportFakta.forsoegt}</span></article></div><section className="ejer-support-kilder"><header><b>Dokumenterede kilder fra seneste forslag</b><span>{supportKilder.length ? `${supportKilder.length} fundet` : "Ingen tilstrækkelige kilder"}</span></header>{supportKilder.map((kilde) => <article key={kilde.id}><div><b>{kilde.titel}</b><span>Vidensversion {kilde.aktuelVersion || "ukendt"} · relevant for {kilde.relevanteVersioner}</span></div><span className={`fc-pill ${kilde.publikum === "kunde_godkendt" ? "ok" : "warn"}`}>{kilde.publikum === "kunde_godkendt" ? "Godkendt til kundesvar" : "Kun intern viden"}</span><small>Kilde: {kilde.kilde} · Gennemgået af {kilde.gennemgaaetAfNavn || "ikke registreret"}</small></article>)}{!supportKilder.length && <p className="ejer-infoboks"><EjerIkon navn="info" size={18}/> Der er endnu ikke genereret et forslag med relevante, godkendte kilder. Ukendt grundlag vises som ukendt og udløser næste spørgsmål.</p>}</section></>}<div className="ejer-oplysningstabel">{oplysninger.filter((post) => post.id !== "saelgerBaggrund" && (!supportVisning || !["version", "produktversion"].includes(post.id))).map((post) => <article key={post.id}><span className={"ejer-oplysning-status " + post.tilstand}><EjerIkon navn={post.tilstand === "mangler" ? "question" : "check"} size={16}/></span><div><b>{post.label}</b><strong>{post.vaerdi}</strong><small>Kilde: {post.kilde || "Ikke registreret"}</small></div></article>)}</div><label className="ejer-sagsbaggrund">{supportVisning ? "Intern supportbaggrund" : "Sælgerens ekstra baggrund"}<textarea rows="5" value={baggrund} onChange={(event) => setBaggrund(event.target.value)} placeholder="Tilføj intern kontekst uden at ændre kundens oplysninger"/><small>Intern sagsoplysning · sendes aldrig til kunden</small></label><button type="button" className="fc-btn" onClick={gemBaggrund} disabled={arbejder || baggrund === (saelgerBaggrund?.vaerdi || "")}>Gem intern baggrund</button>{valgt.dokumenter && <div className="ejer-oplysningsdokumenter"><b>Kilder og dokumenter</b>{poster(valgt.dokumenter).map((dokument) => <span key={dokument.id}><EjerIkon navn="document" size={17}/>{dokument.navn || dokument.id}</span>)}</div>}{internSag && <section className="ejer-intern-struktur"><h3>Intern sag og mappe</h3><p>Modpart: <b>{modpart}</b>. Sagen opretter ikke en kunde.</p><p>Foreslået mappe: <b>{valgt.foreslaaetMappe || "Domicil"}</b> · gælder kun denne sag.</p><div className="fc-actions"><button type="button" className="fc-btn" onClick={() => udfoer(() => opdaterKommunikationsklassifikation({ traadId: valgt.id, sagstype: "intern", delingsstatus: valgt.delingsstatus, internMappe: valgt.foreslaaetMappe || "Domicil", forventetRevision: valgt.revision }))}>Godkend sortering</button><button type="button" className="fc-btn" onClick={() => udfoer(() => opdaterKommunikationsklassifikation({ traadId: valgt.id, sagstype: "intern", delingsstatus: valgt.delingsstatus === "delt" ? "privat" : "delt", internMappe: valgt.internMappe || "", forventetRevision: valgt.revision }))}>{valgt.delingsstatus === "delt" ? "Gør personlig" : "Del med Jørn"}</button></div></section>}</section>}
        </div>
