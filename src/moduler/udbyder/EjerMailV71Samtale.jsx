...
function SvarReviewDialog({ kladde, svartekst, onLuk, onGodkend, arbejder }) {
  ...
  return <div className="ejer-modalbaggrund"><section ref={ref} className="ejer-mail-reviewdialog" role="dialog" aria-modal="true" aria-labelledby="svar-review-titel">
    ...
    <dl><dt>Fra</dt><dd>{kladde.fra}</dd><dt>Til</dt><dd>{kladde.til}</dd><dt>Emne</dt><dd>{kladde.emne}</dd></dl>
...
export default function EjerMailV71Samtale({ valgt, bruger, hent, params, lokaleKladder, setLokaleKladder, tilbage }) {
  ...
  const [aiInstruks, setAiInstruks] = useState(""); const [mobilpanel, setMobilpanel] = useState("samtale");
  ...
  const foreslaaSvar = async () => {
    ...
    if (r.ok) { setAiInstruks(""); setAktivFane("ai-chat"); }
  };
  const indsætForslag = () => {
    ...
    <div className={"ejer-mail-fokus mobil-" + mobilpanel}>
      ...
      <aside className="ejer-design-kort ejer-mail-svarfokus"><header><span className="ejer-ikonfelt"><EjerIkon navn="sparkles"/></span><div><h2>Svar og AI</h2><small>Fælles internt arbejdsrum · eksterne tjenester ikke tilsluttet</small></div></header>
        ...
        <div className="ejer-mail-ai-paneler">
          {aktivFane === "svarudkast" && <section id="panel-svarudkast" role="tabpanel" aria-labelledby="fane-svarudkast" className="ejer-mail-ai-panel"><div className="ejer-svarmeta"><span><b>Fra</b>{kladde?.fra || "info@veyrosystems.com"}</span><span><b>Til</b>{kladde?.til || valgt.kontaktEmail || "—"}</span><span><b>Emne</b>{kladde?.emne || "Re: " + valgt.emne}</span></div><div className="ejer-mail-editor"><div className="ejer-mail-formatlinje" aria-label="Tekstformatering"><button type="button" aria-label="Fed">B</button><button type="button" aria-label="Kursiv"><i>I</i></button><button type="button" aria-label="Understreget"><u>U</u></button><span/><button type="button" aria-label="Punktopstilling">☷</button></div><textarea aria-label="Svarudkast" value={svartekst} onChange={(event) => setLokaleKladder((gammel) => ({ ...gammel, [valgt.id]: event.target.value }))} rows="12"/></div>{beskidt && <small className="ejer-mail-ugemt">Ugemte ændringer · fanevalg bevarer teksten, og Tilbage er beskyttet.</small>}<p className="ejer-mail-vedhaeftningslinje"><EjerIkon navn="attachment" size={17}/> {kladde?.vedhaeftninger?.length ? kladde.vedhaeftninger.map((fil) => fil.navn).join(", ") : "Ingen vedhæftninger"}</p><div className="ejer-svarhandlinger"><button type="button" className="fc-btn" onClick={gemSvar} disabled={arbejder || !svartekst.trim()}>Gem kladde</button>{valgt.sagstype === "salg" && <Link className="fc-btn" to="/main/tilbud">Opret tilbud</Link>}{kladde?.status === "godkendt" ? <button type="button" className="fc-btn fc-btn-primary" onClick={afsendSvar} disabled={arbejder}>Afsend godkendt svar</button> : <button ref={reviewKnapRef} type="button" className="fc-btn fc-btn-primary" onClick={() => setVisReview(true)} disabled={arbejder || beskidt || !kladde || kladde.status !== "kladde"}>Gennemse og send</button>}</div><p className="fc-hint">Gennemgang godkender kun den viste kundetekst. Afsendelse er en særskilt handling og er blokeret, når Microsoft 365 ikke er tilsluttet.</p></section>}
