 ...
 */
export function byggGrundlag({ bookingId, kundeId, division = "faelles", linjer = [], udarbejdetAf }, nu = Date.now()) {
  if (!bookingId) throw new Error("byggGrundlag: et grundlag hører til et forløb.");
...
export function kanGodkende(grundlag, { etaper = [], bruger } = {}) {
  ...
  const mine = etaper.filter((e) => e?.bookingId === grundlag.bookingId);
