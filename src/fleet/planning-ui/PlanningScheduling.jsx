...
export function LegacyPlanningScheduling({ state, setState, onOpenLive, calendarOnly = false, createLocalUrl = null }) {
  ...
  const openWindow = (query) => {
    const destination = createLocalUrl
      ? createLocalUrl({ view: "planlaegning", search: query })
      : new URL(`planning-demo.html?${query}`, window.location.href).href;
