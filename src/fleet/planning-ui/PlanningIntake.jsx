...
import {
  BESTILLINGSPERIODEART, BESTILLINGSPERIODENIVEAU, DUBLETBESLUTNING, DUBLETTYPE, IMPORTKILDE, IMPORTTRIN, INTAKESTATUS, KOLONNEFELTER, STOPTYPE,
  ...
  opretOpgaverFraImportRaekker, parseCsv, parseIndsatTabel, sendTilDagsplan,
...
import {
  ...
  STANDARD_IMPORTKOLONNER, standardImportCsvDataUrl,
} from "./planning-import-template.js";
...
function ManuelDialog({ skabeloner, materialer, onClose, onSave }) {
  ...
  const [form, setForm] = useState({ eksternReference: "", navn: "", kunde: "Demo-modtager", prioritet: "NORMAL", rutetype: "Service", medarbejderRef: "", koeretoejRef: "", skabelonId: "", periodeart: BESTILLINGSPERIODEART.UDEN_DATO_OENSKE, periodeniveau: BESTILLINGSPERIODENIVEAU.OENSKET, oensketDato: "", oensketTid: "", oensketVindueFra: "", oensketVindueTil: "", oensketUge: "38", oensketUgeAar: "2032", periodeFra: "", periodeTil: "", stop: [tomtStop()] });
  const gem = () => {
    let nummer = 0;
  ...
  return <PlanningWorkPanel className="pi-large-panel" title="Opret opgave" eyebrow="Samme validering som import" onClose={onClose} draft={Boolean(form.navn || form.eksternReference)} actions={<><button className="pu-btn pu-btn-quiet" type="button" onClick={onClose}>Luk panelet</button><button className="pu-btn pu-btn-primary" type="button" onClick={gem}>Gem lokal opgave</button></>}>
    ...
    <fieldset className="pi-stop-form"><legend>Bestillerens oprindelige dato- og tidsønske</legend><div className="pi-form-grid"><label>Periodetype<select value={form.periodeart} onChange={(e) => setForm({ ...form, periodeart: e.target.value })}>{Object.values(BESTILLINGSPERIODEART).map((value) => <option key={value}>{value}</option>)}</select></label><label>Betydning<select value={form.periodeniveau} onChange={(e) => setForm({ ...form, periodeniveau: e.target.value })}><option value={BESTILLINGSPERIODENIVEAU.OENSKET}>Ønsket tidspunkt</option><option value={BESTILLINGSPERIODENIVEAU.SKAL_OVERHOLDES}>Skal overholdes</option></select></label>{[BESTILLINGSPERIODEART.DATO, BESTILLINGSPERIODEART.DATO_TID, BESTILLINGSPERIODEART.TIDSVINDUE].includes(form.periodeart) && <label>Ønsket dato<input type="date" value={form.oensketDato} onChange={(e) => setForm({ ...form, oensketDato: e.target.value })}/></label>}{form.periodeart === BESTILLINGSPERIODEART.DATO_TID && <label>Ønsket tid<input type="time" value={form.oensketTid} onChange={(e) => setForm({ ...form, oensketTid: e.target.value })}/></label>}{form.periodeart === BESTILLINGSPERIODEART.TIDSVINDUE && <><label>Ønsket fra<input type="time" value={form.oensketVindueFra} onChange={(e) => setForm({ ...form, oensketVindueFra: e.target.value })}/></label><label>Ønsket til<input type="time" value={form.oensketVindueTil} onChange={(e) => setForm({ ...form, oensketVindueTil: e.target.value })}/></label></>}{form.periodeart === BESTILLINGSPERIODEART.ISO_UGE && <><label>ISO-uge<input type="number" min="1" max="53" value={form.oensketUge} onChange={(e) => setForm({ ...form, oensketUge: e.target.value })}/></label><label>År<input type="number" min="2000" value={form.oensketUgeAar} onChange={(e) => setForm({ ...form, oensketUgeAar: e.target.value })}/></label></>}{form.periodeart === BESTILLINGSPERIODEART.DATO_INTERVAL && <><label>Periode fra<input type="date" value={form.periodeFra} onChange={(e) => setForm({ ...form, periodeFra: e.target.value })}/></label><label>Periode til<input type="date" value={form.periodeTil} onChange={(e) => setForm({ ...form, periodeTil: e.target.value })}/></label></>}</div><p className="pu-help">Ønsket tidspunkt er en præference. Skal overholdes er et bindende krav. Uden datoønske opfinder ingen dato.</p></fieldset>
...
function ImportDialog({ demo, eksisterende, onClose, onImport }) {
  ...
  const valider = () => {
    let nummer = 0;
    const opgaver = opretOpgaverFraImportRaekker(parser?.raekker || [], mapping, { tenantRef: "tenant-fiktiv-ui-demo", kilde, batchId: "batch-lokal-preview", filnavn, importeretMs: DEMO_IMPORTTID_MS + 180000, idGenerator: (p) => `import-${p}-${++nummer}`, udfoerelsesskabeloner: demo.skabeloner, materialer: demo.materialer });
  ...
  return <PlanningWorkPanel className="pi-import-panel" title="Importér opgaver" eyebrow="Lokal browserbehandling" onClose={onClose} actions={<button className="pu-btn pu-btn-quiet" type="button" onClick={onClose}>Luk import</button>}>
    <p className="pi-safety-note"><strong>Lokal prototype – brug kun syntetiske testdata.</strong> Filer forlader ikke din browser.</p>
    <section className="pi-template-download" aria-labelledby="importskabelon-titel"><div><h3 id="importskabelon-titel">Download standard importskabelon</h3><p>Én-stop og flerstop med afhentning før levering. Kan åbnes direkte i Excel.</p></div><a className="pu-btn pu-btn-primary" download={IMPORTSKABELON_FILNAVN} href={standardImportCsvDataUrl()}>Download CSV</a><small>{IMPORTSKABELON_ENCODING} · semikolon ({IMPORTSKABELON_SEPARATOR}) · kun syntetiske eksempeldata</small><details><summary>Se kolonner og formater</summary><div className="pi-template-columns">{STANDARD_IMPORTKOLONNER.map((kolonne) => <span key={kolonne.felt}><strong>{kolonne.overskrift}{kolonne.paakraevet ? " *" : ""}</strong><small>{kolonne.format}</small></span>)}</div></details><p>Ægte .xlsx er udskudt. Der oprettes ikke en falsk Excel-fil og ingen ny dependency.</p></section>
...
export default function PlanningIntake({ planlaegningspulje = null, setPlanlaegningspulje = null }) {
  ...
  const [valgtId, setValgtId] = useState(() => typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("opgaveId"));
  const [valgte, setValgte] = useState([]);
  const [dialog, setDialog] = useState(null);
  const [kladder, setKladder] = useState({});
  const [lokalBesked, setLokalBesked] = useState("");
  ...
  const openRow = (opgaveId) => { setDialog(null); setValgtId(opgaveId); };
  const openDialog = (type) => { setValgtId(null); setDialog(type); };
  return <div className="pu-view pi-view" data-view="opgaver"><div className="pu-view-title"><div><span className="pu-eyebrow">Intake og datakontrol</span><h1>Opgaver</h1><p>Lokal prototype – brug kun syntetiske testdata. Filer forlader ikke din browser.</p></div><div className="pi-inline-actions"><button type="button" className="pu-btn pu-btn-quiet" onClick={() => openDialog("import")}>Importér opgaver</button><button type="button" className="pu-btn pu-btn-primary" onClick={() => openDialog("manuel")}>Opret opgave</button></div></div>
    {lokalBesked && <p className="ps-local-message" role="status">{lokalBesked}</p>}
    ...
    <section className="pu-card pi-inbox"><div className="pi-toolbar"><label>Søg<input type="search" value={soegning} onChange={(e) => setSoegning(e.target.value)} placeholder="Opgave, kunde eller reference" /></label><label>Sortér<select value={sortering} onChange={(e) => setSortering(e.target.value)}><option value="dato">Dato</option><option value="navn">Opgavenavn</option></select></label><button type="button" className="pu-btn pu-btn-quiet" disabled={!valgte.length} onClick={masseGodkend}>Godkend valgte</button></div><div className="pi-table-wrap"><table><thead><tr><th><span className="pu-sr-only">Vælg</span></th><th>Status</th><th>Dato og tid</th><th>Opgave</th><th>Kunde/modtager</th><th>Stop</th><th>Krav</th><th>Kilde</th><th>Fund</th></tr></thead><tbody>{filtrerede.map((o) => <tr key={o.id} className="pi-clickable-row" tabIndex="0" data-draft={Boolean(kladder[o.id])} onClick={() => openRow(o.id)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openRow(o.id); } }}><td><input aria-label={`Vælg ${o.navn}`} type="checkbox" checked={valgte.includes(o.id)} onClick={(event) => event.stopPropagation()} onChange={(e) => setValgte(e.target.checked ? [...valgte, o.id] : valgte.filter((id) => id !== o.id))} /></td><td><Status status={o.status} />{kladder[o.id] && <span className="pu-badge" data-tone="estimated">Kladde</span>}</td><td>{o.stop[0]?.dato}<small>{o.stop[0]?.tidskrav?.art || "ukendt"}</small></td><td><button className="pu-link" type="button" onClick={(event) => { event.stopPropagation(); openRow(o.id); }}>{o.navn || "Navn mangler"}</button><small>{o.eksternReference}</small></td><td>{o.kunde}</td><td>{o.stop.length}<small>{o.stop.map((s) => `${s.type} · ${s.lokation.by}`).join(" / ")}</small></td><td>{o.udfoerelsessnapshot ? `Skabelon v${o.udfoerelsessnapshot.skabelonVersion}` : "Ingen"}<small>{o.udfoerelsessnapshot ? "Dokument · foto · spørgsmål" : "—"}</small></td><td>{o.kilde}<small>{o.kildeMetadata.batchId || "Manuel"}</small></td><td>{o.valideringsfund.length + (o.dublet.type !== DUBLETTYPE.INGEN ? 1 : 0)}</td></tr>)}</tbody></table></div></section>
    <SkabelonVaerksted skabeloner={skabeloner} setSkabeloner={setSkabeloner} materialer={materialer} setMaterialer={setMaterialer} />
    {valgt && <PlanningWorkPanel title={valgt.navn || "Ufuldstændig opgave"} eyebrow={valgt.eksternReference} onClose={() => setValgtId(null)} onOpenNewTab={() => aabnNyFane(valgt)} draft={Boolean(kladder[valgt.id])} actions={<>{valgt.status === INTAKESTATUS.AFVIST ? <button type="button" className="pu-btn pu-btn-quiet" onClick={() => opdater(genaabnOpgave(valgt))}>Genåbn</button> : <button type="button" className="pu-btn pu-btn-quiet" onClick={() => opdater(afvisOpgave(valgt, "Afvist i lokal prototype"))}>Afvis</button>}<button type="button" className="pu-btn pu-btn-quiet" disabled={valgt.valideringsfund.length > 0} onClick={() => opdater(godkendOpgave(valgt, { tidspunktMs: DEMO_IMPORTTID_MS + 360000 }))}>Godkend</button><button type="button" className="pu-btn pu-btn-primary" disabled={valgt.status !== INTAKESTATUS.KLAR_TIL_PLANLAEGNING || pulje.some((p) => p.id === valgt.id)} onClick={() => send(valgt)}>Send til dagsplan</button></>}><div className="pu-badge-row"><Status status={valgt.status} /><span className="pu-badge" data-tone="neutral">{valgt.kilde}</span>{valgt.dublet.type !== DUBLETTYPE.INGEN && <span className="pu-badge" data-tone="warn">{valgt.dublet.type} dublet</span>}</div><dl className="pu-detail-grid"><div><dt>Kunde</dt><dd>{valgt.kunde}</dd></div><div><dt>Rutetype</dt><dd>{valgt.rutetype || "Ikke angivet"}</dd></div><div><dt>Stop</dt><dd>{valgt.stop.length}</dd></div><div><dt>Varighed</dt><dd>{valgt.stop.reduce((s, x) => s + (x.estimeretVarighedMin || 0), 0)} min.</dd></div></dl><label className="pi-draft-note">Lokal kladdenote<textarea value={kladder[valgt.id] || ""} onChange={(event) => setKladder({ ...kladder, [valgt.id]: event.target.value })} placeholder="Bevares kun i den aktuelle lokale session"/></label><h3>Stop og lokation</h3><div className="pi-review-list">{valgt.stop.map((s) => <article key={s.id}><div><strong>{s.type} · {s.navn}</strong><small>{s.lokation.adresse}, {s.lokation.postnummer} {s.lokation.by}, {s.lokation.land}</small></div><span>{s.originalVarighed || "Varighed mangler"}</span><small>{s.lokation.status} · ikke geokodet</small></article>)}</div><h3>Valideringsfund</h3>{valgt.valideringsfund.length ? valgt.valideringsfund.map((f) => <p className="pi-finding" key={`${f.kode}-${f.sti}`}>{f.tekst}</p>) : <p className="pu-help">Ingen hårde strukturelle fund.</p>}{valgt.udfoerelsessnapshot && <ExecutionMobilePreview compact />}</PlanningWorkPanel>}
