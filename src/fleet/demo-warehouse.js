/* src/fleet/demo-warehouse.js
 * Demo-data til Warehouse.
 *
...
if (import.meta.env?.DEV) {
  ...
  for (const k of DEMO_KASSER) {
    if (!typer.has(k.type)) {
      console.warn(`demo-warehouse: ${k.id} har type "${k.type}", som ikke findes i DEMO_KASSETYPER.`);
    }
    if (!pladser.has(k.hjemPladsId)) {
      console.warn(`demo-warehouse: ${k.id} har hjemPladsId "${k.hjemPladsId}", som ikke findes.`);
    }
    ...
    if (k.status === "udlaant" && k.pladsId) {
      console.warn(`demo-warehouse: ${k.id} er udlånt OG står på en plads. Reglerne afviser den.`);
    }
    if (PAA_LAGER.includes(k.status) && !k.pladsId) {
      console.warn(`demo-warehouse: ${k.id} er ${k.status} men står ingen steder.`);
    }
    if (k.pladsId) {
      if (!pladser.has(k.pladsId)) {
        console.warn(`demo-warehouse: ${k.id} står på "${k.pladsId}", som ikke findes.`);
      }
      ...
      if (brugtePladser.has(`${k.pladsId}|${k.id}`)) {
        console.warn(`demo-warehouse: ${k.id} står to gange.`);
      }
  ...
  for (const u of DEMO_KASSEUDLAAN) {
    if (!kasser.has(u.kasseId)) {
      console.warn(`demo-warehouse: udlån ${u.id} peger på kasse "${u.kasseId}", som ikke findes.`);
    }
    if (!(u.til >= u.fra)) {
      console.warn(`demo-warehouse: udlån ${u.id} slutter før det begynder.`);
    }
    if (!u.sagsnummer) {
