    ...
    (async () => {
      try {
        if (demo) {
          ...
          const bytes = createOrderPdfBytes(order, supplier, { ...(tenant || {}), procureAppUrl, procureReceiptQr: { size: modules.size, data: Array.from(modules.data, Boolean) } });
      ...
      } catch { if (active) setMessage({ tone: "bad", text: "Ordre-PDF'en kunne ikke hentes fra den godkendte revision." }); }
    })();
