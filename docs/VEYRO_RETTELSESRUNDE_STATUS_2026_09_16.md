# Veyro-rettelsesrunde — efterprøvbar kravstatus 16. september 2026
...
Aktuel vurdering: **24 implementerede, 12 delvise, 0 ikke implementerede og 0
blokerede**.
...
`adc63b7dedc812011f2af25e813c59726629ec21`. Denne rundes lokale commits er
`c13c6d25ae97512b4a695690d216902a3ee22757`,
`541cc20ce3b6de5d2586ed33ec07226b6d9a827d`,
`2ec39146b79435a03275b25bf142620fc14db292`,
`0119e8e1add2f434cefe5619257b51aae2523676`,
`42eb6452b217e4ee0a2e911e7a48c55096256336`,
`dfc95fc5a2d7a4b338a35cdc843a27972282ec14`,
`ef7d9f7e4231a1bff4b9d80e8166d94f553fd1f2` og
`7ae8ea6906839a410e201a6c998e5246e53af071`. Browserbeviset er optaget på
`ef7d9f7e…`; den efterfølgende commit ændrede kun README/testforventninger.
Root-lint, FLEET-lint, designgate 11/11, FLEET 207/207,
functions-paritet 29/29, root- og FLEET-build, Rules-/platformsgate
4.649/4.649 og den integrerede browser-QA er beskrevet i
`docs/VEYRO_FLEET_NAVIGATION_QA_2026_09_16.md`.
...
| FC-05 Match- og modulfilter — PDF/audit FC-05 | Implementeret | Match/modulfilter og fast nyeste-rækkefølge findes. | I integreret root-app gav `Matchet` fire serverposter, `Flere moduler` kun `FC-FILTER-FLERE`, og `Mangler match` kun `FC-MASSE-MANGLER-GRUNDLAG`. | Readmodel over serverposter i RTDB-emulator. | Tenantafgrænset læsning i normal session. | Browser-QA `dc0c29b`; aktuelt v3-bevis. | Ingen særskilt mangel. |
...
| FC-07 Tre paneler, scroll, justering/hukommelse — PDF/audit FC-07 | Implementeret | Tre uafhængigt scrollende paneler, splittere, minimumsbredder og kontekstlagring. | Første separator tastaturprøvet 22→23 %; begge separatorer pointertrukket 23/48→28/42 % og bevaret 28/42 % efter reload. Alle tre paneler rullet uafhængigt. Fire viewports uden vandret dokumentscroll. | `localStorage`-visningspræference, ikke forretningsgem. | Tenant-/bruger-/skærmscope. | Browser-QA `dc0c29b`; aktuelt v3-bevis. | Ingen særskilt mangel. |
...
| FL-05 Fælles autoritativt enhedsregister — PDF/audit FL-05 | Implementeret | FLEET og PLANNING bruger samme `koeretoejer`-post uden IndexedDB-fallback i servergrenen. Alle fælles statusser round-tripper; `kmStand`/`driftstimer` er autoritativ; uvedkommende patches kan ikke tilbagerulle dem; samtidige måler-/statusændringer afvises med bevaret input. Oprettelse bruger samme optimistiske, autoritative writer. | NB-003 blev åbnet med `udeAfDrift` og 412.980 km trods en 412.000-profilværdi, gemt og genindlæst uden ændring. En nyere servermåler 413.100 km blev bevaret; en modstridende ændring mod 413.180 km blev afvist med input bevaret. Både eksisterende og ny fælles enhed blev vist i PLANNING. | Autoritativ læse-/skrivekilde: tenantens RTDB-emulatorregister; PLANNING læser samme post. Lokale sager er fortsat særskilt mærket prototype. | Normal login, FLEET-modul, tenant og permission bevist; Rules har det nødvendige `kaldenavn`-indeks. | `c13c6d25`, `0119e8e1`; writer 8/8; FLEET 207/207; browser-QA `ef7d9f7`; Rules 4.649/4.649. | Permanent billedupload afventer Storage-kontrakt, men ændrer ikke den verificerede fælles tekst-/stamdatakilde. |
...
| FL-07 Indberetninger i tre paneler — PDF/audit FL-07 | Implementeret | Tre paneler, resize, minimumsbredder, huskning, scroll og semantik. | Begge separatorer pointertrukket 26/28→31/33 % og bevaret efter reload; liste, detalje og sagspanel rullet uafhængigt. Integreret ved fire viewports; filterretur bevist. | Lokal FLEET-repository. | Root-moduladgang bevist. | Browser-QA `dc0c29b`; aktuelt v3-bevis. | Serverpersistens er ikke påstået. |
...
| FL-09 Arbejdskødialoger/drag — PDF/audit FL-09 | Implementeret | Flytbar detaljedialog, genbrugt sagsmappe og mobil fuldskærm. | Ved 1920×1080 flyttede fysisk musetræk dialogen 100 px og beholdt den inden for arbejdsfladen; aktuelle billeder findes ved 1920×1080 og 1440×900. Ved 390×844 var dialogen 390×844 med intern auto-scroll. | Lokal repository/route. | Root-moduladgang bevist. | FLEET 206/206 + browser-QA `dc0c29b`; `97858a5`. | Autoritativ mutation er ikke påstået. |
...
| REG-01 Rules-/sikkerhedsgate — PDF/audit REG-01 | Implementeret | Relevante Rules/Functions-kontrakter dækker tenant, rolle, revision, samtidighed, idempotens, fælles `fleetProfil` og modulopdelt anden godkendelse. | Ikke et UI-krav; adgangsgrænserne er desuden browserprøvet. | Isolerede Auth/RTDB/Functions/Storage-emulatorer; ingen ekstern service. | Anonym, uden moduladgang, tilladt bruger, egenkontrol og anden godkender er prøvet. | Aktuel fuld gate 4.649/4.649 i 905 suites, 0 fejlet; `7ae8ea6`. | Ingen rest for det aktuelle kodegrundlag. Nye fremtidige serverfunktioner skal fortsat have egne grønne sikkerhedsbeviser. |
| REG-02 Samlet tværmodulregression — PDF/audit REG-02 | Delvist | Aktuelle fælles klientkontrakter, lint, design, builds, Functions-paritet, FLEET-tests og Rules-gate består. | Integreret FLEET/Fakturacenter/PLANNING, adgang, handlinger, 40 viewport- og 160 layoutkombinationer bevist med 0 runtimefejl. | Den aktuelle fulde Rules-gate er grøn; de endnu ikke implementerede adaptere har naturligt ikke slutbevis. | Anonym, uden FLEET-adgang, tilladt tenantbruger og to fakturagodkendere prøvet. | Browser-QA `ef7d9f7`; FLEET 207/207; design 11/11; paritet 29/29; Rules 4.649/4.649. | Slutregression efter FC-06 og FL-13–18s resterende backend/integrationer. |
| REG-03 Komplet viewport-/zoommatrix — PDF/audit REG-03 | Implementeret | Responsiv shell, komprimeret mobilchrome, mobil undermenu og scoped overflowrettelser er implementeret. | 40/40 for ti routes ved 1920×1080, 1440×900, 390×844 og 360×800; 160/160 i normal/kompakt menu × 100/125 % zoom; dirty-dialog ved alle fire viewports. TEST-mærkning forblev synlig. | Emulatorsourcede auth/tenant/fælles data og tydeligt mærkede lokale FLEET-fixtures. | Tilladt session; adgangsskærme også fotograferet. | `c03f64c1`; browser-QA `dc0c29b`; aktuelt v3-bevis. | Ingen særskilt mangel. |

## Forklaring af ændringen fra den oprindelige audit

Den oprindelige audit talte 14 implementerede, 16 delvise, 5 ikke
implementerede og 1 blokeret. Den aktuelle 24/12-opgørelse følger disse
...
- fra blokeret til implementeret: REG-01, fordi den aktuelle uændrede
  sikkerhedsgate nu er genkørt og består 4.649/4.649 på slutgrundlaget.

Resultatet er matematisk 24 implementerede og 12 delvise. Den nye integrerede
...
på dette tidspunkt ikke opfyldte sin fælles læse-/skrivekilde. V2-reviewet
havde tilsvarende ret i, at R1/R2 gjorde det efterfølgende grundlag utilstrækkeligt.
FL-05 står først som implementeret efter den fælles kilde, R1/R2-rettelserne
`c13c6d25ae97512b4a695690d216902a3ee22757`, oprettelsesrettelsen
`0119e8e1add2f434cefe5619257b51aae2523676` og browserbeviset på
`ef7d9f7e4231a1bff4b9d80e8166d94f553fd1f2` for sikker redigering og
oprettelse af den samme RTDB-post i FLEET og PLANNING.
...
`esbuild spawn EPERM` i den normale proces-sandbox og Java-emulatorens
tidligere opstartsfejl er dokumenteret særskilt. Slutgaten blev kørt med den
proceslokale JDK 21 og bestod 4.649/4.649; ingen global Java-/Windows-
indstilling blev ændret. Den første slutkørsel havde tre gamle
dokumentations-/forventningsafvigelser, ikke sikkerhedsfejl; genkørslen var
grøn. Ingen test blev svækket eller sprunget over. Ingen eksisterende
brugerdata blev migreret, ingen ekstern tjeneste blev aktiveret, og ingen
produktionstilstand blev ændret.
