...
| 4. Fælles design | `AppShell`, `procure-v2.css`, fælles tokens | Én fælles shell, afgrænsede styles, semantiske tokens, responsivt layout og store mobile trykflader | Designtest 11/11; screenshots ved 360, 390 og 1440 px uden vandret sideoverflow | Visuel hardwarekontrol på fysisk telefon mangler |
...
| 6. Mail og PDF | `ordreMailSend`, `ordrePdfHent`, `procure-pdf.js` | Aktiv Send-handling, revisionslås, kanonisk PDF, arkiv, faktisk multipart-vedhæftning, SHA-256 og status `ukendt` ved tvetydig timeout | Kontrolleret transport; vedhæftningspayload og identiske bytes testet; konkret PDF-indhold testet | Rigtige Mailgun-/kundecredentials ikke tilsluttet |
| 7. Webshop og beskyttet adgang | `procureWebshopCredentialGem/Hent`, `procureWebshopBestillingRegistrer`, AES-GCM-modul | Tenantafgrænset, serverkrypteret legitimation; kun administrator/ansvarlig indkøber; kortlivet svar uden browserlagring; revisions- og idempotenskontrol | Faktisk emulatorflow: krypteret roundtrip, køber afvist, hemmelighed ikke i audit, åbning ændrede ikke ordrestatus, gentagelse gav ingen dublet | Ingen rigtig webshop eller leverandørkonto afprøvet |
| 8. Firmakort og Fakturacenter | `procureWebshopBestillingRegistrer`, fælles `fakturaer`, Fakturacenter-link | Firmakort registreres uden kortnummer/CVV, lagerflow kan afsluttes mens økonomistatus forbliver `afventerDokumentation`; fakturaer forbliver fælles | Faktisk emulatorordre via webshop med firmakortstatus og dubletværn; sammenhængende syntetisk fakturaflow | Ingen bank-, kort- eller bogføringsintegration; ingen betaling udført |
| 9. Tilbage til rettelse og fysisk retur | linjeafgørelser, `procureVareReturneringRegistrer`, fælles ordretilstandsmaskine, modtagelseskorrigering | Retur/afvisning kræver begrundelse; relevante ændringer kræver ny vurdering; fysisk retur, kreditnota og refundering er adskilte hændelser | Faktisk emulatorflow: fysisk retur af 1 rulle, dubletværn, tilknyttet/godkendt kreditnota og netto 8.856 kr. uden omskrivning af modtagelsen | Fysisk returtransport og kortrefundering er ikke eksternt integreret |
...
| 11. Mobilmodtagelse | `MobileReceiptScreen.jsx`, fem modtagelses-/filcallables, Storage-regler | Flere delleverancer, rest, beskadiget/afvist, korrektion, flere dokumenter, type/størrelse/magic-bytes, tenantadgang og genåbning | Database/Storage-emulator og faktisk handlerprøve med ny session og fremmed tenant | OCR og fysisk kamerafotografering ikke afprøvet |
...
| 14. Verifikation og aflevering | tests, browserscripts og denne rapport | Lint, build, design, fokuserede tests, fuld regel-/regressionssuite, Auth/Functions/Database-emulator og screenshots | Se `PROCURE_FOLLOWUP_VERIFICATION.md` for kommandoer og resultater | Deployment er bevidst ikke udført |
...
| Kort og sand kvittering | mobilens `Mine indkøb`/kvittering | Viser kort `IND-ÅÅÅÅ-XXXXXX`, sendt/rest-linjer, leverandørstatus og siger eksplicit, at leverandøren endnu ikke har modtaget ordren | Ingen kendt kodemangel |
...
| Modtagelse, PDF, QR og fysisk retur | Storage-/mail-/QR-/returcallables | To aktive bilag kan genåbnes; identiske PDF-bytes; QR kræver aktivt Tilføj; fysisk retur og efterfølgende kreditnota spores særskilt | Fysisk kamera og returtransport ikke afprøvet |
