...
| 6. Mail og PDF | `ordreMailSend`, `ordrePdfHent`, `procure-pdf.js` | Aktiv Send-handling, revisionslås, kanonisk PDF, arkiv, faktisk multipart-vedhæftning, SHA-256 og status `ukendt` ved tvetydig timeout | Aktuel QR-holdig PDF: 30.736 bytes og SHA-256 `39f2b98e…9421` identisk i preview, testtransport og arkiv; vedlagt browser-PDF blev også visuelt kontrolleret og QR-afkodet | Rigtige Mailgun-/kundecredentials ikke tilsluttet |
...
| 11. Mobilmodtagelse | `MobileReceiptScreen.jsx`, fem modtagelses-/filcallables, Storage-regler | Flere delleverancer, rest, beskadiget/afvist, korrektion, flere dokumenter, type/størrelse/magic-bytes, tenantadgang og genåbning | PDF-QR → login-retur → 8 leveret/1 beskadiget/1 afvist = 6 godkendt + to PNG-bilag; anden session genåbnede bilag og modtog rest 4; anden tenant afvist | OCR og fysisk kamerafotografering ikke afprøvet |
...
| Ordre-PDF-QR til modtagelse | `procure-pdf.js`, `ordrePdfHent`, `MobileReceiptScreen.jsx` | QR fra selve PDF-renderingen blev afkodet til korrekt ordre, returnerede efter normalt login, gav ingen automatisk modtagelse og respekterede tenantadgang | Fysisk kamera ikke afprøvet; softwareafkodning og browserflow bestået |
