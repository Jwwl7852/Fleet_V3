...
| **F.2+** | Supplier Portal — resten (dokumentdeling-udvidelse, Cloud Functions, portal-UI, admin-UI) | NOT STARTED | Fundet undervejs: den eksisterende sikre dokument-arkitektur (Skive 4C) er HARDKODET til `parentType: "faktura"` alene (`firebase.rules.json`) — kode-kommentaren kalder selv udvidelse til andre parentTypes "en senere skive på samme fundament". Skal udvides (ikke genopfindes) for at bære opgave-vedhæftede, leverandør-synlige fotos (§7) |
