...
| **F.2+** | Supplier Portal — resten (dokumentdeling-udvidelse, portal-UI, admin-UI) | NOT STARTED | Dokumentdeling er en EGEN, isoleret security-skive før portal-UI'et bygges (produktejerens eksplicitte krav 2026-08-31) — den eksisterende sikre dokument-arkitektur (Skive 4C) er HARDKODET til `parentType: "faktura"` alene (`firebase.rules.json`); kode-kommentaren kalder selv udvidelse til andre parentTypes "en senere skive på samme fundament". Ingen direkte Storage-adgang, public URLs eller en separat usikker portal-filvej — udvidelsen skal følge samme principper som 4C (tenant-isoleret sti, server-side grant-tjek, eksplicit `visibleToSupplier`, short-lived links, audit af link-udstedelse) |
